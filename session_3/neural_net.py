...
def save_parameters(name, value, epoch):
    ...
    with open('G:/Project/python/DSLab Training 2020/session3/model/' + filename, 'w') as f:
        f.write(string_form)


def restore_parameters(name, epoch):
    filename = name.replace(':', '-colon-') + '-epoch-{}.txt'.format(epoch)
    with open('G:/Project/python/DSLab Training 2020/session3/model/' + filename) as f:
        lines = f.readlines()
...
def test():
    ...
    test_data_reader = DataReader(
        data_path='G:/Project/python/DSLab Training 2020/datasets/20news-bydate/test_tf_idf.txt',
        batch_size=50,
        vocab_size=vocab_size
    )
    mlp = MLP(
